...
class DescribeAsyncCommonSubjectTestMixin:
    ...
    class TestIsCommonSubjectDeferred(AsyncCommonSubjectTestMixin):
        common_subject = lambda_fixture('get_next_val')

        class AssertNotDeferred:
            """Test context mixin verifying traits of an undeferred common_subject
            """
            async def it_calls_common_subject_automatically(self, call_common_subject):
                expected = 2  # it ought to have already been called once,
                              # so we get value #2 when calling it below
                actual = await call_common_subject()
                assert expected == actual
        ...
        class AssertDeferred:
            """Test context mixin verifying traits of n deferred common_subject
            """
            async def it_does_not_call_common_subject_automatically(self, call_common_subject):
                expected = 1  # it ought not to have been called,
                              # so we get value #1 when calling it below
                actual = await call_common_subject()
                assert expected == actual
    ...
    class TestPreconditions(AsyncCommonSubjectTestMixin):
        ...
        class TestOverridingPreconditionsFixture:
            # Grab a val at the beginning
            first_val = lambda_fixture(lambda get_next_val: get_next_val(), async_=True)

            # Grab a val late (but should still be before common_subject_rval!)
            late_val = pytest.mark.late(lambda_fixture(lambda get_next_val: get_next_val(), async_=True))
        ...
        class TestPreconditionFixture:
            # Grab a val at the beginning
            first_val = precondition_fixture(lambda get_next_val: get_next_val(), async_=True)

            # Grab a val late (but should still be before common_subject_rval!)
            late_val = pytest.mark.late(precondition_fixture(lambda get_next_val: get_next_val(), async_=True))
